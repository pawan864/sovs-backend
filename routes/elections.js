...
router.get('/:id/eligible-voters', protect, authorize('admin'), async (req, res) => {
  try {
    const election = await Election.findById(req.params.id).populate('eligibleVoters', 'name voterId aadhaarNumber eciCardNumber voterLocation');
    if (!election) return res.status(404).json({ success: false, message: 'Election not found' });
...
router.post('/', protect, authorize('admin'), async (req, res) => {
  try {
    ...
    const election = await Election.create({ title, description, startDate: new Date(startDate), endDate: new Date(endDate), totalVoters: totalVoters || 0, candidates: candidates || [], location: location || {}, visibleTo: visibleTo || ['voter', 'dm', 'sdm', 'cdo'], createdBy: req.user._id });
...
router.post('/:id/eligible-voters', protect, authorize('admin'), async (req, res) => {
  try {
    ...
    if (election.eligibleVoters.some(id => id.toString() === voterId)) return res.status(400).json({ success: false, message: 'Voter already in eligible list' });
...
router.delete('/:id/candidates/:candidateId', protect, authorize('admin'), async (req, res) => {
  try {
    ...
    if (election.candidates.length === before) return res.status(404).json({ success: false, message: 'Candidate not found' });
    await election.save();
    res.json({ success: true, data: election });
  } catch (error) { res.status(500).json({ success: false, message: error.message }); }
});
